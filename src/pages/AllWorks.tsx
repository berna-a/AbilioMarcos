  ...
  return (
    <div className="border-b border-border pb-5 mb-5 last:border-b-0 last:pb-0 last:mb-0">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full text-left group">
        <span className="text-[12px] tracking-[0.25em] uppercase text-foreground group-hover:text-foreground transition-colors duration-300">{title}</span>
        <ChevronDown className={`w-3 h-3 text-foreground transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
      </button>
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
            <div className="mt-3 space-y-1">
              {options.map((opt) => (
                <button key={opt.value} onClick={() => onToggle(opt.value)} className={`block w-full text-left py-1 text-[13px] tracking-[0.03em] transition-colors duration-300 ${selected.includes(opt.value) ? "text-foreground font-medium" : "text-foreground hover:text-foreground"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
  ...
  const filtered = useMemo(() => {
    let result = artworks.filter((a) => {
      ...
      if (filters.format.length) {
        const w = Number(a.custom_width_cm) || 0;
        const h = Number(a.custom_height_cm) || 0;
        if (!w || !h) return false;
        const isSquare = Math.abs(w - h) / Math.max(w, h) <= 0.05;
        const isVertical = !isSquare && h > w;
        const isHorizontal = !isSquare && w > h;
        const fmt: 'vertical' | 'horizontal' | 'square' = isSquare ? 'square' : isVertical ? 'vertical' : 'horizontal';
  ...
  const filterContent = (
    <div>
      <div className="flex items-center justify-between mb-8">
        <span className="text-[12px] tracking-[0.3em] uppercase text-foreground">{t.allWorks.filter}</span>
        {activeCount > 0 && (
          <button onClick={clearAll} className="text-[12px] tracking-[0.1em] text-foreground hover:text-foreground transition-colors duration-300 underline underline-offset-2">{t.allWorks.clearAll}</button>
        )}
      </div>
      <FilterSection
      ...
      <div className="pt-28 md:pt-36 pb-24 md:pb-36 px-6 md:px-10">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="md:hidden mb-8">
            <button onClick={() => setMobileFiltersOpen(true)} className="flex items-center gap-2 text-[12px] tracking-[0.2em] uppercase text-foreground hover:text-foreground transition-colors duration-300 border border-border px-4 py-2.5">
              ...
              <>
                ...
                <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "tween", duration: 0.35 }} className="fixed bottom-0 left-0 right-0 z-50 bg-background border-t border-border max-h-[80vh] overflow-y-auto px-6 py-6 md:hidden">
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-[12px] tracking-[0.3em] uppercase text-foreground">{t.allWorks.filter}</span>
                    <button onClick={() => setMobileFiltersOpen(false)} className="text-foreground hover:text-foreground transition-colors"><X className="w-4 h-4" /></button>
                  </div>
                  {filterContent}
          ...
          <div className="flex gap-12 lg:gap-16">
            ...
            <div className="flex-1 min-w-0">
              <div className="mb-8 flex items-center justify-between">
                <p className="text-[12px] tracking-[0.15em] text-foreground">
                  {loading ? t.allWorks.loading : `${filtered.length} ${filtered.length === 1 ? t.allWorks.work : t.allWorks.works}`}
                </p>
                <div className="flex items-center gap-2">
                  <span className="text-[12px] tracking-[0.15em] text-foreground hidden sm:inline">{t.allWorks.sort}</span>
                  <select
                    value={sort}
                    onChange={(e) => { const v = e.target.value as SortOption; track('sort_used', { sort_value: v }); setSort(v); }}
                    className="text-[12px] tracking-[0.05em] text-foreground bg-transparent border border-border px-3 py-1.5 focus:outline-none cursor-pointer hover:text-foreground transition-colors"
                  >
                    {sortOptions.map((opt) => (
              ...
              {!loading && filtered.length === 0 ? (
                <div className="py-20 text-center">
                  <p className="text-sm text-foreground">{t.allWorks.noMatch}</p>
                  {activeCount > 0 && (
                    <button onClick={clearAll} className="mt-4 text-[12px] tracking-[0.15em] uppercase text-foreground hover:text-foreground underline underline-offset-2 transition-colors">{t.allWorks.clearFilters}</button>
                  )}
                </div>
              ) : (
                <div
