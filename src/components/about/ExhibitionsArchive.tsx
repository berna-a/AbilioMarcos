...
type FilterKey = "all" | "solo" | "group" | "fair";
...
const ExhibitionsArchive = () => {
  ...
  const filters: { key: FilterKey; label: string }[] = [
    { key: "all", label: t.aboutPage.all },
    { key: "solo", label: t.aboutPage.solo },
    { key: "group", label: t.aboutPage.group },
    { key: "fair", label: t.aboutPage.fairs },
  ];

  const filtered = filter === "all" ? exhibitions : exhibitions.filter((e) => e.type === filter);
  const grouped = filtered.reduce<Record<string, ExhibitionEntry[]>>((acc, e) => { (acc[e.year] ||= []).push(e); return acc; }, {});

  return (
    <motion.section className="mb-24 md:mb-32" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
      <div className="flex items-baseline justify-between mb-10 pb-3 border-b border-gallery-border">
        <p className="text-[10px] tracking-[0.3em] uppercase text-muted-foreground">{t.aboutPage.exhibitionsTitle}</p>
        <div className="flex gap-4">
          {filters.map((f) => (
            <button key={f.key} onClick={() => setFilter(f.key)} className={`text-[11px] tracking-[0.1em] uppercase transition-colors duration-200 ${filter === f.key ? "text-foreground" : "text-muted-foreground/50 hover:text-muted-foreground"}`}>{f.label}</button>
          ))}
        </div>
      </div>
      <div className="space-y-8">
        {Object.entries(grouped).map(([year, entries]) => (
          <div key={year} className="grid grid-cols-12 gap-4">
            <div className="col-span-3 md:col-span-2"><p className="text-sm font-light text-foreground/60">{year}</p></div>
